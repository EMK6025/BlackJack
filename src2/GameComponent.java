...
public class GameComponent extends JComponent implements MouseListener {
  ...
  public void mousePressed(MouseEvent e) {
    ...
    if (clickX >= 50 && clickX <= 200 && clickY >= 300 && clickY <= 450) {
      ...
      if (choice == 0) {
        ...
        playerBet = 1;
        playerBalance -= 1;
        System.out.println("No valid bet selected. Defaulting bet to 1.");
      }

      System.out.println("You placed a bet of " + playerBet + ". If you win, your balance increases by " + (playerBet * 2) +
              "; if you lose, it decreases by " + playerBet + ".");

      playChipSound();
      Tester.newGame.initiateGame(); // Begin the round after placing a bet
    }
  }
  ...
  /**
   * Plays a sound associated with placing chips on the table.
   */
  public static void playChipSound() {
    try {
      InputStream soundStream = new FileInputStream("sounds/chipsSettle.wav");
      // Audio setup commented out per original code
      // AudioStream audio = new AudioStream(soundStream);
      // AudioPlayer.player.start(audio);
    } catch (IOException e) {
      // If unable to play sound, do nothing special here
    }
  }
}